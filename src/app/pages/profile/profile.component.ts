...
export class ProfileComponent {
  ...
  ngOnInit() {
    ...
    console.log(
      'profile.ts - ngOnInit() - user.getCurrentUser() ',
      this.user.username,
      this.user.id,
      this.createdAt
    );
    if (this.user) {
      ...
      if (this.user.birthday) {
        this.birthday =
          this.datePipe.transform(this.user.birthday, 'MMMM d, y') || '';
      }
      if (this.user.createdAt) {
        this.createdAt =
          this.datePipe.transform(this.user.createdAt, 'MMMM, y') || '';
  ...
  getPostsByUserId() {
    ...
    this.postService.getPostsByUserId(this.userId).subscribe((data) => {
      ...
      this.postsUser = data;
      console.log(
        'profile.ts - getPostsByUser() - this.postsUser',
        this.postsUser
      );
    });
  }
